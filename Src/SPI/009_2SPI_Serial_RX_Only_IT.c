...
//			while(GPIO_ReadFromInputPin(GPIOA, GPIO_PIN_NO_0)); //wait until button has been released
			...
			while(rxContFlag == SET) {
				waitForTxRx = 1;
				SPI_SendDataIT(&SPI2Handle, &dummy_write, 1);
				while(waitForTxRx);
				waitForTxRx = 1;
				SPI_ReceiveDataIT(&SPI2Handle, &readByte, 1);
				while(waitForTxRx);
			}
...
void SPI_ApplicationEventCallback(SPI_Handle_t *pSPIHandle, uint8_t appEvent) {
	if(appEvent == SPI_EVENT_RX_COMPLETE) {
		...
		receive++;
		waitForTxRx = 0;
	} else if(appEvent == SPI_EVENT_TX_COMPLETE) {
		transmit++;
		waitForTxRx = 0;
